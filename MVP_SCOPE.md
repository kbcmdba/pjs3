...
- **Auth and workspace-bootstrap design** — Better-Auth with the organization plugin (surfaced as "Workspace"). Drizzle owns all DDL including Better-Auth's tables (single source of schema truth). MVP auth = email + password; OAuth + 2FA roadmapped post-MVP; magic links explicitly rejected. Email verification is a hard gate — workspace + Owner membership materialize atomically at first verified sign-in (not at signup). JWT sessions with user-configurable TTL (4h default). Login prompts for workspace selection when the user has multiple memberships; workspace switching requires full re-authentication, not just token reissue. Per-request `workspaceId` and role come from the JWT only — body/URL/header workspaceId fields are ignored. See `docs/adr/0003-auth-and-workspace-bootstrap.md` for the full picture, alternatives considered, and remaining open sub-questions. Resolved 2026-04-26.
